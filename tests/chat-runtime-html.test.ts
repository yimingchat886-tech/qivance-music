...
test("renders local-only runtime chat HTML with timeline controller", () => {
  ...
  assert.match(html, /function renderAt/);
  assert.match(html, /function trackYAt/);
  assert.match(html, /requestAnimationFrame/);
  assert.match(html, /class="chat-viewport"/);
  assert.match(html, /class="chat-track"/);
  assert.match(html, /class="bottom-spacer"/);
  assert.match(html, /CHAT_BOTTOM_RESERVE_PX\s*=\s*300/);
  ...
  assert.match(html, /class="receipt-text">已读<\/span>/);
